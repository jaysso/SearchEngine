...
def get_content(soup_page, word_count_dict, doc_id):  # Pass in a soup object --> this function will extract the text content in the json and put then into a giant string
    ...
    lst_word += [stemmer.stem(i) for i in tknz_text if i != ''] * 2
    ...
    lst_word += [stemmer.stem(i) for i in tknz_text if i != ''] * 8
    lst_word_unweighted += [stemmer.stem(i) for i in tknz_text if i != '']
    ...
    lst_word += [stemmer.stem(i) for i in tknz_text if i != ''] * 6
    lst_word_unweighted += [stemmer.stem(i) for i in tknz_text if i != '']
    ...
    lst_word += [stemmer.stem(i) for i in tknz_text if i != ''] * 4
    lst_word_unweighted += [stemmer.stem(i) for i in tknz_text if i != '']
...
def dict_to_file(index_dict):
    ...
    with open('indexer.json', 'r') as r:
        ...
        print(type(read))
    '''
    with open('D:\Codes\CS121\indexes_test_ver2.json', 'w') as j:
...
def run():
    ...
    with open('D:\Codes\CS121\word_count_page_ver2.json', 'w') as j:
        json.dump(list(word_cout_dict.items()), j)
    with open('D:\Codes\CS121\doc_id_ver2.json', 'w') as j:
        json.dump(list(doc_dict.items()), j)
    # write the doc_id_dictionary to a json file

if __name__ == '__main__':
    run()
    #/Users/wangtianhe/Desktop/CS121/DEV
    #/Users/wangtianhe/Desktop/CS121/Indexing/inverted_index.py
    #wats return is: [['categori', 0.3, [1]], ['mainten', 0.9, [1, 1, 2]], ['sli', 0.3, [0]]]
    #                   word    tf_idf index_list

    #indexes: {"the": {"0": [14, [1, 1]],
